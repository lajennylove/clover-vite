...
class Vite implements Htmlable
{
    ...
    public function __invoke($entrypoints, $buildDirectory = null)
    {
        ...
        if ($this->isRunningHot()) {
            return new HtmlString(
                $entrypoints
                    ->prepend("@vite/client")
                    ->map(
                        fn($entrypoint) => $this->makeTagForChunk(
                            $entrypoint,
        ...
        foreach ($entrypoints as $entrypoint) {
            ...
            foreach ($chunk["imports"] ?? [] as $import) {
                $preloads->push([
                    $import,
                    $this->assetPath(
                        "{$buildDirectory}/{$manifest[$import]["file"]}"
                    ),
                    $manifest[$import],
                    $manifest,
                ]);
                ...
                foreach ($manifest[$import]["css"] ?? [] as $css) {
                    $partialManifest = Collection::make($manifest)->where(
                        "file",
                        $css
                    );
        ...
        [$stylesheets, $scripts] = $tags
            ->unique()
            ->partition(fn($tag) => str_starts_with($tag, "<link"));
        ...
        return new HtmlString(
            $preloads->join("") . $stylesheets->join("") . $scripts->join("")
        );
    }
    ...
    protected function makeTagForChunk($src, $url, $chunk, $manifest)
    {
        ...
        if ($this->isCssPath($url)) {
            return $this->makeStylesheetTagWithAttributes(
                $url,
                $this->resolveStylesheetTagAttributes(
                    $src,
                    $url,
                    $chunk,
                    $manifest
    ...
    {
        $attributes = $this->resolvePreloadTagAttributes(
            $src,
            $url,
            $chunk,
            $manifest
        ...
        return "<link " .
            implode(" ", $this->parseAttributes($attributes)) .
            " />";
    }
    ...
    protected function resolveScriptTagAttributes($src, $url, $chunk, $manifest)
    {
        ...
        foreach ($this->scriptTagAttributesResolvers as $resolver) {
            $attributes = array_merge(
                $attributes,
                $resolver($src, $url, $chunk, $manifest)
            );
        }
     ...
     * @return array|false
     */
    protected function resolvePreloadTagAttributes(
        $src,
        $url,
        $chunk,
        $manifest
        ...
        $attributes = $this->isCssPath($url)
            ...
            : [
                ...
                "nonce" => $this->nonce ?? false,
                "crossorigin" =>
                    $this->resolveScriptTagAttributes(
                        $src,
                        $url,
                        $chunk,
                        $manifest
        ...
        foreach ($this->preloadTagAttributesResolvers as $resolver) {
            if (
                false ===
                ($resolvedAttributes = $resolver($src, $url, $chunk, $manifest))
            ) {
                return false;
            }
    ...
    protected function parseAttributes($attributes)
    {
        return Collection::make($attributes)
            ...
            ->flatMap(
                fn($value, $key) => $value === true ? [$key] : [$key => $value]
            )
            ->map(
...
HTML
    ...
    public function content($asset, $buildDirectory = null)
    {
        ...
        if (!is_file($path) || !file_exists($path)) {
            throw new Exception(
                "Unable to locate file from Vite manifest: {$path}."
            );
        }
    ...
    protected function manifest($buildDirectory)
    {
        ...
        if (!isset(static::$manifests[$path])) {
            ...
            static::$manifests[$path] = json_decode(
